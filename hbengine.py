...
class Polling:
	def __init__(self,wuInterval,syncInterval,pollInterval):
		...
		self.arme = 0
		self.alarmDevices = [11,15,17,21,31,44,48]
		self.allDevices = [5,11,15,17,19,21,31,44,48]
	...
	def start(self):
		...
		while True:
			...
			if sync == self.syncInterval:
				url = "http://"+HCL_USER+":"+HCL_PASS+"@"+HCL_IP+"/api/globalVariables/arme"
				resp = self.getUrl(url)
				if (resp and resp.ok):
			...
			if uid in ("3761E99E", "11FB229E", "4102E39E","F1CC67AE") :
				...
				headers = {"Content-Type" : "application/x-www-form-urlencoded;charset=utf-8"}
					...
					for dev in self.alarmDevices:
						url = "http://"+HCL_USER+":"+HCL_PASS+"@"+HCL_IP+"/api/callAction?deviceID=" + str(dev) + "&name=setArmed&arg1=0&arg2=7911"
